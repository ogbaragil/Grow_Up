...
const isValidMonthKey = (key) => /^\d{4}-\d{2}$/.test(String(key || ""));
const safeMonthKey = (key) => isValidMonthKey(key) ? key : currentMonthKey();
const monthLabel = (key) => {
  const [y,m] = safeMonthKey(key).split("-").map(Number);
...
const isFutureMonth = (key) => safeMonthKey(key).localeCompare(currentMonthKey()) > 0;
...
function normalizeGrowState(rawState = {}) {
  const base = { ...EMPTY_STATE, ...(rawState && typeof rawState === "object" ? rawState : {}) };
  base.currency = isSupportedCurrency(base.currency) ? base.currency : "USD";
  base.selectedMonth = safeMonthKey(base.selectedMonth);
  base.monthSnapshots = base.monthSnapshots && typeof base.monthSnapshots === "object" && !Array.isArray(base.monthSnapshots) ? base.monthSnapshots : {};
...
function upcomingTransactions(transactions, days) {
  ...
  for (const transaction of transactions || []) {
    ...
    const parsedEndDateLimit = transaction.endDate ? new Date(transaction.endDate) : null;
    const endDateLimit = parsedEndDateLimit && !Number.isNaN(parsedEndDateLimit.getTime()) ? parsedEndDateLimit : null;
